...
class TransformersClassifierHandler(BaseHandler, ABC):
    ...
    def thread(self, text, query):
        ...
        with ThreadPoolExecutor(max_workers=2) as pool:
            futures['query'] = pool.submit(self.model.encode, query)
            futures['text'] = pool.submit(self.model.encode, text)
        tensor = util.pytorch_cos_sim(futures['query'].result(), futures['text'].result())
        return float(tensor.numpy()[0][0])
